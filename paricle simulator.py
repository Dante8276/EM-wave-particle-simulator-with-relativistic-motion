...
C= 3*(10**8)                                                           ## speed of light stored as constant
fEx = Expression("0*x+0*y+0*z+0*t",["x","y","z","t"])                 ## forces stored here as Expression
...
q=1                                                                     ## charge and mass initialised as unit
m=1
Ex="0"                                                            ## forces stored as string 
Ey="0"
Ez="0"
Bx="0"
By="0"
...
choice = "rel"
relchoice = ""
origin=vector(0,0,0)                                      ## origin declared
def make_axes(length):        ## creates axis
...
def modifyB(x,y,z,t):           ## modifies B acoording to the input given
    return vector(fBx(x,y,z,t),fBy(x,y,z,t),fBz(x,y,z,t))

def modifya_nonrelativistic(V,x,y,z,t):    ## modifies a acoording to the input given in non_relativistic condition
    return vector((q/m)*(V.y*fBz(x,y,z,t)-V.z*fBy(x,y,z,t)+fEx(x,y,z,t)),(q/m)*(-1*V.x*fBz(x,y,z,t)+V.z*fBx(x,y,z,t)+fEy(x,y,z,t)),(q/m)*(V.x*fBy(x,y,z,t)-V.y*fBx(x,y,z,t)+fEz(x,y,z,t)))

def dotp(V,x,y,z,t):                    ## returns V.E
    return V.x*fEx(x,y,z,t)+V.y*fEy(x,y,z,t)+V.z*fEz(x,y,z,t)

def modifya_relativistic(V,x,y,z,t,gamma,force):     ## modifies a acoording to the input given in relativistic condition
    if(force == "E"):
        return vector((q*fEx(x,y,z,t))/(m*gamma),0,0)
    if(force == "B"):
        return vector((q*fBz(x,y,z,t)*V.y)/(m*(gamma**2)),-1*(q*fBz(x,y,z,t)*V.x)/(m*(gamma**2)),0)

def modifygamma(V):        ## modifies gamma acoording to the input
    return (1/math.sqrt(1-(V.x**2+V.y**2+V.z**2)/(C**2)))

def simulate_nonrelativsitic():           ## simulates particle in non_relativistic condition
...
def simulate_relativistic(force):               ## simulates particle in relativistic condition
...
def create_menu():                ## creates input menu
    menu = Tk()
    menu.title('Particle Simulator')
   
    def openCalculator(x):        ## creates input window for forces
        root = Tk()
        ...
        #e.insert(0, "")
        def efupdate(exp):        ## updates final field
            current=ef.get()
            ef.delete(0, END)
            ef.insert(0, str(current) + exp)
        
        def button_click(exp):    ## updates field on button click
            current = e.get()
            e.delete(0, END)
            e.insert(0, str(current) + exp)
                
        def operation_click(op):    ## updates field on operation click
            current = e.get()
            print(e.get())
            e.delete(0, END)
        ...
        def main_clear():           ## cleares input field
            ef.delete(0,END)
            
        def button_clear():
            e.delete(0, END)

        def button_equal():         ## stores data on expression and closes the window
    ...
    def openinputfield(sys):        ## creates window to choose which forces to input
        global choice
        choice = sys
        if(sys == "nonrel"):
            nrelroot = Tk()
            ...
            def setnrelv():   ## confirms all the values and closes window
    ...
    def update_label():     ## updates values to be shown on screen
